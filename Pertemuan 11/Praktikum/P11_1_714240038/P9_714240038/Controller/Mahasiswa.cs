...
namespace P9_714240038.Controller
{
    internal class Mahasiswa
    {
        ...
        public bool Insert(M_mahasiswa mahasiswa)
        {
            ...
            try
            {
                ...
                var cmd = new MySqlCommand("INSERT INTO t_mahasiswa (npm, nama, angkatan, alamat, email, nohp) " +
                "VALUES (@npm, @nama, @angkatan, @alamat, @email, @nohp)"// Pastikan ada objek koneksi jika diperlukan konstruktor
                );
        ...
        public bool Update(M_mahasiswa mahasiswa, string npm_mhs)
        {
            ...
            try
            {
                ...
                var cmd = new MySqlCommand("UPDATE t_mahasiswa SET nama=@nama, angkatan=@angkatan, alamat=@alamat, email=@email, nohp=@nohp " +
                    "WHERE npm=@npm"// Pastikan ada objek koneksi jika diperlukan konstruktor
                );
        ...
        { bool status = false;
            ...
            { 
                ...
                var cmd = new MySqlCommand("DELETE FROM t_mahasiswa WHERE" +
               " npm=@npm"); cmd.Parameters.AddWithValue("@npm", npm_mhs);
                koneksi.ExecuteQuery(cmd); status = true;
                MessageBox.Show("Data berhasil dihapus", "Informasi",
                MessageBoxButtons.OK, MessageBoxIcon.Information); } catch (Exception e)
            { 
